# Himeno Sena

 ![LoveSena](https://img.shields.io/badge/Love-SENA-e6ad88?style=flat-square) [![Build](https://github.com/biyuehu/HimenoSena/actions/workflows/build.yml/badge.svg)](https://github.com/biyuehu/HimenoSena/actions/workflows/build.yml) ![License](https://img.shields.io/badge/License-GPL--3.0-blue.svg?style=flat-square) ![GitHub contributors](https://img.shields.io/github/contributors/biyuehu/HimenoSena?) ![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white) ![Codeberg](https://img.shields.io/badge/Codeberg-2185D0?style=flat-square&logo=codeberg&logoColor=white)

<!-- markdownlint-disable-next-line -->
Hi there! This is a character website project designed for <font color="#E6AD88" size="5rem">**the best girl in the world** — <font size="6rem">**Himeno Sena!**</font></font>

---

- Welcome any people who liked **Himeno Sena** to leave own messages, it'll display on the website. More details refer to [this](/docs/messages.md).
- 欢迎任何喜欢**姬野星奏**的人在这里留下自己的话，这将会显示在网站上。具体细节请参考 [留言](/docs/messages.md)
- **姫野星奏**が好きな方なら誰でも、ここにメッセージを残すことができます。サイト上に表示されます。詳細は [こちら](/docs/messages.md) をご参照ください。

- Contributions to the blacklist are welcome — covering anyone who defames, insults, attacks, mocks, or slanders **Himeno Sena** and related content. More details refer to [Add Blacklist](/docs/blacklist.md).
- 欢迎帮助项目收录诋毁、辱骂、攻击、讽刺、抹黑**姬野星奏**及其相关内容的 spammer。具体细节请参考 [添加黑名单](/docs/blacklist.md)
- **姫野星奏**およびその関連コンテンツを誹謗・侮辱・攻撃・中傷する者のブラックリスト収録にご協力ください。詳細は [ブラックリストに追加](/docs/blacklist.md) をご参照ください。

- For any suggestions regarding this project or the website, feel free to send them to [i@himeno-sena.com](mailto://i@himeno-sena.com). Friendly messages of any kind are also welcome — replies are casual and whenever the mood strikes.
- 对于本项目、网站有任何好的建议都可以发送到邮箱 [i@himeno-sena.com](mailto://i@himeno-sena.com)，同时邮箱欢迎任何友好的消息，随缘回复
- このプロジェクトやサイトへのご意見・ご提案は [i@himeno-sena.com](mailto://i@himeno-sena.com) までお気軽にどうぞ。どんな気軽なメッセージも歓迎します。返信はその気になったときに、ゆるりと。

---

- **Himeno Sena** (姫野星奏/ひめのせな) is a main character in the Japanese visual novel [Koi x Shin Ai Kanojo (恋×シンアイ彼女)](http://ustrack.amusecraft.com/koikake/).
- You can make suggestions or report any issues about the website or project on the [GitHub Issues](https://github.com/biyuehu/HimenoSena/issues).
- Of course, if you are a developer and find any bugs or have any ideas, you can contribute the project to improve it by creating pull requests.

## About the website

Please refer to the article [我做了一个姬野星奏的专属网站](/docs/about.md) (Language: Chinese).

## Contributors (星厨名单)

[![Contributors](https://contrib.rocks/image?repo=biyuehu/himenosena)](https://github.com/biyuehu/HimenoSena/graphs/contributors)

## Stacks

- Pages: Lit & Web Component (Fucking Vue, React, Angular, Svelte and Solid)
- Development Environment: Deno (Fucking Node.Js)
- Editor: Zed (Fucking VSCode)
- CICD: GitHub Actions
- Version Control: Git
- Domain: Tencent Cloud (Fucking Aliyun)
- DNS: Cloudflare (Fucking Tencent)

## Announcement and thanks

- The background image is from the official CG.
- Thanks to [火神80](https://github.com/huoshen80) for providing the server.
- The background music:
  - flower -piano arrangement- - 水月陵
  - 風の止まり木 - 水月陵
  - GLORIOUS_DAYS (Short) - yuiko
  - 東の空から始まる世界 - yuiko

## License

Under the `GPL-3.0-or-later` license.
