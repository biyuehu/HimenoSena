...
const normalizeRawRecord = (
  record: Partial<RawBlacklistRecord>,
): RawBlacklistRecord => ({
...
const resolveName = async (
  record: RawBlacklistRecord,
  previous: BlacklistRecord[],
) => {
  const oldRecord = previous.find((item) =>
    item.platform === record.platform && item.id === record.id
  );
...
const updateBlacklist = async () => {
  const rawRecords =
    (await readJson<Partial<RawBlacklistRecord>[]>(RAW_BLACKLIST_PATH)).map(
      normalizeRawRecord,
    );
  const previousRecords = await readJson<BlacklistRecord[]>(BLACKLIST_PATH)
    .catch(() => []);
